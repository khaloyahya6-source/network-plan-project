...
class RF_Optimizer:
    ...
    def objective_function(self, p):
        ...
        for layer_key, sectors in layers.items():
            ...
            for i, (t_idx, s_idx, rsrp) in enumerate(sectors):
                ...
                # Quality Area (SINR priority)
                total_sinr_fitness += np.sum((rsrp >= rsrp_thresh) & (layer_sinrs[i] >= sinr_thresh))

                if t_idx not in tower_best_rsrp: tower_best_rsrp[t_idx] = rsrp
                else: tower_best_rsrp[t_idx] = np.maximum(tower_best_rsrp[t_idx], rsrp)

            if len(tower_best_rsrp) > 1:
                stack = np.stack(list(tower_best_rsrp.values()))
        ...
        # Geometric & Overshooting Constraints
        ...
        for i, rsrp in enumerate(all_rsrp):
            ...
            for s2 in range(s_idx + 1, 3):
                diff = abs(az - params[t_idx, s2, 0])
                diff = min(diff, 360 - diff)
                if diff < 45: intra_penalty += (45 - diff) * 10000

        fitness = total_sinr_fitness - total_interference_penalty - intra_penalty - savage_overshoot_penalty
        return -fitness
    ...
    def run_optimization(self, n_particles=60, max_iter=100):
        ...
        print(f"Executing Multi-Tech PSO Engine ({dim}D, {n_particles} particles)...")
